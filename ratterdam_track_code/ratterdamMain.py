...
#            print(incomingData)
 ...
 #           print(incomingData)
            ...
            self.TaskObj.updateRules_RF(incomingData)
            #if self.TaskObj.currComm == 10:
            
            
            command = np.int8(self.TaskObj.nextState_RF())
...
class Foraging():
    ...
    def __init__(self, nActive, nChoices, nReplace, nAlleys = 17):
        ...
        self.beltwayAlleys = [i-1 for i in [1, 5, 7, 8, 10, 11, 14, 16, 17]]
        self.currComm  = self.beltwayAlleys[0]
        self.initRules_DF = {'A':4, 'B':5, 'C': 6}
        # Initialize active alleys and texture pattern
        self.alleyRules = self.initializeRules_Beltway()
        self.nextTextures_DF = [self.selectNewStimuli(x) for i,x in enumerate(self.texturePattern)]
        self.printStatus_RF()
        self.beltwayCycler = itertools.cycle(self.beltwayAlleys)
    ...
    def initializeRules_Beltway(self):
        activeAlleys = np.random.choice(self.beltwayAlleys, self.nActive, replace=False)
        alleyRules = np.array([0]*17)
        alleyRules[activeAlleys] = 3
    ...
    def currCommIncrement_Beltway(self):
        self.currComm = next(self.beltwayCycler)
...
class PositionTracker():
    ...
    def getNewAlley(self, currentAlley, side):
        acceptables = set(forg.beltwayAlleys).difference(set([currentAlley] +
