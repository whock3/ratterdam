...
path <- "C:\\Users\\Ruo-Yah Lai\\Desktop\\My folder\\College\\Junior\\K lab research\\Graphs\\LMER\\20210414-110048_R886D1_3vfilt_.csv"
savepath <- "C:\\Users\\Ruo-Yah Lai\\Desktop\\My folder\\College\\Junior\\K lab research\\Graphs\\LMER\\"
...
for (cellname in unique(df$cell)){
  ...
  for (i in 1:length(histdir)){
    ...
    if ((dir >= 0 && dir <= 45) || (dir > 315 && dir <= 360)){
      histdir[i] <- 1
    }
  ...
  if (!is.null(mod)){
    ...
    a <- confint(mod, method="Wald", level=0.95)
    for (i in 1:length(a[,"2.5 %"])){
      if (is.numeric(a[i,"2.5 %"]) & !is.na(a[i,"2.5 %"])){
        if (!(a[i,"2.5 %"] < 0 & a[i,"97.5 %"] > 0)){
          print(cellname)
          print(rownames(a)[i])
        }
      }
