            ...
            for fname, field in oriendf.groupby('FieldID'):
                ...
                if dirA.shape[0] >= passThresh and dirB.shape[0] >= passThresh:
                    try:
                        ...
                        diff = abs(np.mean(dirA.Rate)-np.mean(dirB.Rate))/np.mean(wf.Rate)
                        biases.append(bias)
                        diffs.append(diff)
...
plt.suptitle(f"Directional Bias vs Directionality, {wnSizeMin}min win, {wnStepMin}min step, all fields pooled across datasets",
             fontsize=30)
ax = fig.axes[0]
ax.scatter(biases,diffs)
...
sbins = np.linspace(6,max(speeds),nbins)
...
#%% Heatmap timexbias with normed abs mean FR diff colored 
