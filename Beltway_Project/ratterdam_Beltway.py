...
import serial, datetime, numpy as np, time, datetime, json, pickle, itertools
...
class RatterdamBeltway():
    ...
    def __init__(self, rat, direction):
        ...
        self.savepath = "C:\\Users\\whock\\Google Drive\\KnierimLab\\Ratterdam\\Beltway\\BehavioralData\\"
        self.rat = rat
        self.direction = direction
        self.currentLap = 0
        self.lapTS = [] # ts in python time for beginning of a lap (actually ITI preceeding lap)
        
        self.beltwayAlleyList = [i-1 for i in [1, 3, 5, 7, 8, 10, 11, 16, 17]]
        ...
        self.textureRDominanceThresh = 0.34
        self.runStatus = 'P'
        self.initTimestamp()
    ...
    def writeDataForGUI(self,ratquit = False):
        ...
        f = open(self.savepath+"beltway_gui_data.txt","w")
        f.write(line)
        f.close()
    ...
    def textureRewardCorrAtAlley(self, alley):
        failToggle = False
        pairs = zip(self.alleys[alley].rewardHistory, self.alleys[alley].textureHistory[:self.currentLap+1]) # dont need currentlap-1 bc slices are right exclusive,
        ...
        for txt in ['A','B','C']:
            if sum(self.alleys[alley].rewardHistory) > 0:
                if txtCounts[txt]/sum(self.alleys[alley].rewardHistory) > self.textureRDominanceThresh:
                    failToggle = True
    ...
    def updateRewards(self):
        ...
        if self.currentLap < self.rewardWindow:
            ...
            possibleAlleys = self.getLessFrequentlyRewardedAlleys(txtByRBalance=True)
            if len(possibleAlleys) > numRewards:
                rewardedAlleys = np.random.choice(possibleAlleys, numRewards,replace=False)
            else:
                ...
                if len(possibleAlleys) > numRewards:
                    ...
                    rewardedAlleys = np.random.choice(self.beltwayAlleyList, numRewards, replace=False)
...
if __name__ == '__main__':
    beltway = RatterdamBeltway('781', 'CW')  
    comm = CommChannel('781', beltway, 'COM5')
    comm.communicate()